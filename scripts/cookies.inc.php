...
function checkcookies() {
    ...
    if (isset($_COOKIE[$controlrow["cookiename"]])) {
        ...
        if (!is_numeric($theuser[0])) {
        	clearcookie($controlrow["cookiename"]);
        	die("Invalid cookie data (Error 1). Please clear cookies and log in again.");
        }
        ...
        if ($row == false) {
        	clearcookie($controlrow["cookiename"]);
        	die("Invalid cookie data (Error 2). Please clear cookies and log in again.");
        }
        
        if ($row["id"] != $theuser[0]) {
        	clearcookie($controlrow["cookiename"]);
        	die("Invalid cookie data (Error 3). Please clear cookies and log in again.");
        }
        
        if (md5($row["password"] . "--" . COOKIE_SALT) !== $theuser[2]) {
        	clearcookie($controlrow["cookiename"]);
        	die("Invalid cookie data (Error 4). Please clear cookies and log in again.");
        }
...
{
	setcookie($cookieName, '', time() - 3600, "/", $controlrow["cookiedomain"], 0);
